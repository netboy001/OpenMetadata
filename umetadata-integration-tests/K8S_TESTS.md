...
UMetadata has two types of Kubernetes integration tests:
...
ENABLE_K8S_TESTS=true mvn test -pl :umetadata-integration-tests -Dtest=K8sIngestionPipelineResourceIT

# Or using system property
mvn test -pl :umetadata-integration-tests -DENABLE_K8S_TESTS=true -Dtest=K8sIngestionPipelineResourceIT
```
...
2. Creates namespace `umetadata-pipelines`
...
5. UMetadata server can now deploy pipelines to K8s
