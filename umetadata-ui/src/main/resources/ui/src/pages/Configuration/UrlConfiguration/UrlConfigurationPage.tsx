...
const UrlConfigurationPage = () => {
  ...
  const [urlConfig, setUrlConfig] =
    useState<UMetadataBaseURLConfiguration>();
  ...
  const fetchUrlConfig = async () => {
    try {
      ...
      setUrlConfig(data.config_value as UMetadataBaseURLConfiguration);
    } catch (error) {
      ...
      <Row className="bg-white p-lg border-radius-sm" gutter={[0, 16]}>
        ...
        <Col span={12}>
          <Row align="middle">
            ...
            <Col span={24}>
              <Typography.Text data-testid="u-metadata-url">
                {urlConfig?.uMetadataUrl
                  ? urlConfig.uMetadataUrl
                  : NO_DATA_PLACEHOLDER}
              </Typography.Text>
            </Col>
