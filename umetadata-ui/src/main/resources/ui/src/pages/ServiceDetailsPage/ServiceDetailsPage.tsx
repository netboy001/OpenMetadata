...
const ServiceDetailsPage: FunctionComponent = () => {
  ...
  const isUMetadataService = useMemo(
    () => decodedServiceFQN === U_METADATA,
    [decodedServiceFQN]
  );
  ...
  const [isLoading, setIsLoading] = useState(!isUMetadataService);
  ...
  const isTestingDisabled = useMemo(
    () =>
      ...
      (isMetadataService && decodedServiceFQN === U_METADATA) ||
  ...
  useEffect(() => {
    if (!isUMetadataService) {
      fetchServicePermission();
    }
  }, [decodedServiceFQN, serviceCategory]);

  useEffect(() => {
    if (isAirflowAvailable && !isUMetadataService) {
