...
import {
  ...
  ImportMode,
  ParsedODCSContract,
  ParsedUMetadataContract,
} from './ODCSImportModal.interface';
import './ODCSImportModal.less';

type ParsedContract = ParsedODCSContract | ParsedUMetadataContract;
...
const ContractImportModal: React.FC<ContractImportModalProps> = ({
  ...
  const parseUMetadataContent = useCallback(
    (parsed: Record<string, unknown>): ParsedUMetadataContract | null => {
  ...
  const parseYamlContent = useCallback(
    (content: string): ParsedContract | null => {
      try {
        ...
        return parseUMetadataContent(parsed);
      } catch {
        return null;
      }
    },
    [isODCSFormat, parseODCSContent, parseUMetadataContent]
  );
        ...
        if (parsed) {
          ...
          setParseError(
            isODCSFormat
              ...
              : t('message.invalid-umetadata-contract-format')
          );
          setSchemaObjects([]);
          setHasMultipleObjects(false);
          setSelectedObjectName('');
  ...
  const renderContractPreview = useCallback(() => {
    ...
    const omContract = parsedContract as ParsedUMetadataContract;
          ...
          <Box sx={{ flex: 1, minHeight: '200px' }}>
            <Typography
              ...
              {isODCSFormat
                ...
                : t(
                    'message.invalid-umetadata-contract-format-required-fields'
                  )}
            </Typography>
