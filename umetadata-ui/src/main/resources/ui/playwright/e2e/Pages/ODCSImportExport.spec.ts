...
test.describe('ODCS Import/Export', () => {
  ...
  // ODCS Import -> UMetadata Export Tests

  test('Import ODCS and export as UMetadata YAML', async ({ page }) => {
    const table = new TableClass();
    const { apiContext } = await getApiContext(page);
    await table.create(apiContext);
    ...
    try {
      ...
      // Export as UMetadata format (native export)
      const downloadPromise = page.waitForEvent('download');
      ...
      // Verify it's a YAML file (UMetadata format)
      expect(filename).toContain('.yaml');
  ...
  test('Import ODCS with description and verify UMetadata export', async ({
    page,
  }) => {
    const table = new TableClass();
    const { apiContext } = await getApiContext(page);
  ...
  test('Verify SLA mapping from ODCS to UMetadata format', async ({
    page,
  }) => {
    const table = new TableClass();
    const { apiContext } = await getApiContext(page);
    ...
    try {
      ...
      // ODCS uses slaProperties, UMetadata uses sla
      expect(exportedYaml).toContain('sla:');
      // UMetadata uses refreshFrequency instead of freshness
...
version: "1.0.0"`;
  ...
  // UMetadata (OM) Format Export/Import Round Trip Tests
  ...
  test('Create contract from UI, export OM format, import with merge, verify data', async ({
    ...
    try {
      ...
      await test.step('Import modified OM YAML with merge option', async () => {
        ...
        await page.getByTestId('import-umetadata-contract-button').click();
  ...
  test('OM format export and import round trip - create, export, delete, reimport', async ({
    ...
    try {
      ...
      // Step 2: Export as UMetadata (OM) format
      const omDownloadPromise = page.waitForEvent('download');
      await page.getByTestId('manage-contract-actions').click();
      await page.waitForSelector('.contract-action-dropdown', {
        state: 'visible',
