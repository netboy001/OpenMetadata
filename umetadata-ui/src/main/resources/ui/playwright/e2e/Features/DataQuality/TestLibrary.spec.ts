  ...
  () => {
    ...
    test('should disable toggle for external test definitions', async ({
      ...
      const externalTest = data.data.find(
        (def: { testPlatforms: string[] }) =>
          !def.testPlatforms.includes('UMetadata')
      );
    ...
    test('should handle external test definitions with read-only fields', async ({
      ...
      await test.step('Create external test definition', async () => {
        ...
        await page.locator('#testPlatforms').click();
        const uMetadataOption = page
          .locator('.ant-select-dropdown:visible')
          .getByTitle('UMetadata');
        await expect(uMetadataOption).toBeVisible();
        await uMetadataOption.click();
      ...
      await test.step(
        ...
        async () => {
          ...
          const mysqlFilterResponse = await apiContext.get(
            '/api/v1/dataQuality/testDefinitions?limit=50&entityType=TABLE&testPlatform=UMetadata&supportedService=Mysql'
          );
          ...
          const postgresFilterResponse = await apiContext.get(
            '/api/v1/dataQuality/testDefinitions?limit=50&entityType=TABLE&testPlatform=UMetadata&supportedService=Postgres'
          );
          ...
          const bigQueryFilterResponse = await apiContext.get(
            '/api/v1/dataQuality/testDefinitions?limit=50&entityType=TABLE&testPlatform=UMetadata&supportedService=BigQuery'
          );
