...
function generateIndexMarkdown(components, stats) {
  ...
  let md = `# UMetadata E2E Test Documentation
...
function renderFileWithCollapse(file, repoBaseUrl) {
  ...
  // Robust path logic for both UMetadata (monorepo) and Collate (separate repo)
  // Goal: relativePath should be 'src/main/resources/ui/...' to match standard docs
  
  let relativePath = file.path;
  const standardPrefix = 'src/main/resources/ui/';

  if (file.path.includes('umetadata-ui/')) {
    // Standard OM case: path contains 'umetadata-ui/src/main/resources/ui/...'
    relativePath = file.path.split('umetadata-ui/')[1];
  } else if (file.path.includes(standardPrefix)) {
    // Collate/Fallback case: path contains 'src/main/resources/ui/...' but maybe not 'umetadata-ui/'
  ...
  // URL Construction assuming 'umetadata-ui' is the root module folder in both repos
  const fileUrl = `${repoBaseUrl}/blob/main/umetadata-ui/${relativePath.startsWith('/') ? relativePath.substring(1) : relativePath}`;
