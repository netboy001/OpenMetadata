...
> This guide will help you run UMetadata UI locally in dev mode.
...
Using the command below, spin up the server locally from the directory `umetadata-dist/target/umetadata-*-SNAPSHOT`
...
## Steps to Run UMetadata UI

Once the node and yarn are installed in the system, you can perform the following steps to run UMetadata UI.
...
**Step 3**: Visit [localhost:3000](http://localhost:3000/) to access the UMetadata UI.

## Developer instructions

This directory contains UI resources and the developer handbook for the UMetadata frontend.
...
First, create a new JSON file for the language you want to add in the `umetadata-ui/src/main/resources/ui/src/locale/languages` directory.
...
cd umetadata-ui/src/main/resources/ui/src/locale/languages
...
UMetadata UI uses Playwright for end-to-end testing. This section will guide you through setting up and running Playwright tests.

### Prerequisites

Before running Playwright tests, ensure you have:

1. **UMetadata Server Running**: The tests require a running UMetadata server instance. You can either:
...
1. **Server Not Running**: Ensure UMetadata server is running and accessible
