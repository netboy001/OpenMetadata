...
```sql
CREATE USER umetadata IDENTIFIED BY 'password';
GRANT SELECT ON ALL TABLES IN SCHEMA PUBLIC TO umetadata;
GRANT SELECT ON ALL TABLES IN SCHEMA V_CATALOG TO umetadata;
```
...
```sql
GRANT SELECT ON ALL TABLES IN SCHEMA V_MONITOR TO umetadata;
```

Note that this setting might only grant visibility to the queries executed by this user. 
A more complete approach will be to grant the `SYSMONITOR` role to the `umetadata` user:

```sql
GRANT SYSMONITOR TO umetadata;
ALTER USER umetadata DEFAULT ROLE SYSMONITOR;
```
...
If you are running the UMetadata ingestion in a docker and your services are hosted on the `localhost`, then use `host.docker.internal:5433` as the value.
$$
