...
**Include**: Explicitly include ML Models by adding a list of regular expressions to the `Include` field. UMetadata will include all ML Models with names matching one or more of the supplied regular expressions. All other ML Models will be excluded.
...
**Exclude**: Explicitly exclude ML Models by adding a list of regular expressions to the `Exclude` field. UMetadata will exclude all ML Models with names matching one or more of the supplied regular expressions. All other ML Models will be included.
...
Optional configuration to soft delete ML Models in UMetadata if the source models are deleted. After deleting, all the associated entities like lineage, etc., with that ML Model will be deleted.
$$
