...
**Include**: Explicitly include API Collections by adding a list of regular expressions to the `Include` field. UMetadata will include all API Collections with names matching one or more of the supplied regular expressions. All other API Collections will be excluded.
...
**Exclude**: Explicitly exclude API Collections by adding a list of regular expressions to the `Exclude` field. UMetadata will exclude all API Collections with names matching one or more of the supplied regular expressions. All other API Collections will be included.
...
Optional configuration to soft delete `API Collections` in UMetadata if the source `API Collections` are deleted. After deleting, all the associated entities like lineage, etc., with that `API Collection` will be deleted.
$$
