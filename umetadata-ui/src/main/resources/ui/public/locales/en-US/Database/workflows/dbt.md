...
Please follow the instructions in <a href="https://docs.getdbt.com/docs/dbt-cloud-apis/service-tokens" target="_blank">dbt Cloud's API</a> documentation to create a dbt Cloud API token.
$$
...
Check out <a href="https://cloud.google.com/iam/docs/keys-create-delete#iam-service-account-keys-create-console" target="_blank">this</a> documentation on how to create the service account keys and download it.
$$
...
If this option is enabled, UMetadata will first search for tables within the same database service if tables are not found it will search across all database services.
...
This options updates the table and column descriptions in UMetadata with descriptions from dbt.
...
However, if the option is enabled, descriptions for all tables and columns in the dbt manifest will be updated in UMetadata.
$$

$$section
### Update Owners $(id="dbtUpdateOwners")

This options updates the table owner in UMetadata with owners from dbt.
...
However, if the option is enabled, owners for all tables and columns in the dbt manifest will be updated in UMetadata.
$$
...
When enabled, UMetadata will fetch tags associated with tables and columns from dbt `manifest.json` and attach them to the corresponding tables in UMetadata.
$$
