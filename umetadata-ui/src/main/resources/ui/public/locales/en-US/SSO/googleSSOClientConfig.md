...
- **Definition:** Maps JWT claims to UMetadata user attributes.
- **Example:** ["email:email", "username:name"]
- **Why it matters:** Controls how user information from Google maps to UMetadata user profiles.
...
- **Why it matters:** Automatically assigns users to existing UMetadata teams based on their Google Workspace attributes during login.
- **How it works:**
...
- **Why it matters:** Allows UMetadata to automatically discover Google's OIDC endpoints.
...
- **Why it matters:** UMetadata uses `client_secret_post` which is supported by Google OAuth.
