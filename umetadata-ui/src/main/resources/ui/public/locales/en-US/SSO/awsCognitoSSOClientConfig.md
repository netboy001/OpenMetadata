...
- **Why it matters:** Tells UMetadata which Cognito User Pool to authenticate against.
...
- **Why it matters:** Determines what user information UMetadata can access.
...
- **Why it matters:** Allows UMetadata to automatically discover Cognito's OIDC endpoints.
...
- **Why it matters:** UMetadata uses `client_secret_post` which is supported by AWS Cognito.
...
- **Definition:** Maps JWT claims to UMetadata user attributes. (Overrides JWT Principal Claims if set)
- **Example:** ["email:email", "username:preferred_username"]
- **Why it matters:** Controls how user information from AWS Cognito maps to UMetadata user profiles.
- **Note:** Format: "umetadata_field:jwt_claim"
...
- **Why it matters:** Automatically assigns users to existing UMetadata teams based on their Cognito user attributes during login.
- **How it works:**
  ...
  - Matches the extracted value(s) against existing team names in UMetadata
...
- **Note:**
  - The team must already exist in UMetadata for assignment to work
