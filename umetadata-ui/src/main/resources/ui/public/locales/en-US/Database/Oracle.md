...
- `GRANT SELECT` on the relevant tables which are to be ingested into UMetadata to the user
```sql
...
You can find further information on the Oracle connector in the <a href="https://docs.wondersgroup.com/connectors/database/oracle" target="_blank">docs</a>.
...
If you are running the UMetadata ingestion in a docker and your services are hosted on the `localhost`, then use `host.docker.internal:1521` as the value.
$$
