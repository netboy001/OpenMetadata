...
Username to connect to TimescaleDB. This user should have privileges to read all the metadata in TimescaleDB.
$$
...
Password to connect to Timescale.
$$
...
Database of the data source. This is optional parameter, if you would like to restrict the metadata reading to a single database. When left blank, UMetadata Ingestion attempts to scan all the databases.
$$

$$section
### Ingest all Database $(id="ingestAllDatabases")

Ingest data from all databases in TimescaleDB. You can use databaseFilterPattern on top of this.
$$
...
SSL Mode to connect to Timescale database. E.g, `prefer`, `verify-ca`, `allow` etc.
$$
$$note
...
$$
$$note
Timescale only needs CA Certificate
$$
