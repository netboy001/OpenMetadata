...
UMetadata is able to send Emails on a various steps like SignUp, Forgot Password , Reset Password, Change Event updates.

Following configuration is needed to allow UMetadata to send Emails.
...
This defines the entity that's sending Email. By default, it's `UMetadata`.

If your company name is `JohnDoe` setting it up will update subject line, content line so that mails have `JohnDoe` inplace of `UMetadata`.
