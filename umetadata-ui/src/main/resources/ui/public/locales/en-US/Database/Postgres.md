...
UMetadata extracts metadata from PostgreSQL using two main sources:
...
**Important**: To extract complete metadata, ensure your UMetadata user has appropriate privileges on the schemas and objects you want to discover. While no explicit `GRANT` is needed for `INFORMATION_SCHEMA` itself, the user must have `USAGE` privilege on schemas to see objects within them:
...
Here's a complete example of setting up a user for UMetadata:

```sql
-- Create user
CREATE USER umetadata_user WITH PASSWORD 'your_password';

-- Grant connection to database
GRANT CONNECT ON DATABASE your_database TO umetadata_user;

-- Grant USAGE on schemas for metadata extraction
-- Repeat for each schema you want UMetadata to discover
GRANT USAGE ON SCHEMA schema1, schema2, schema3 TO umetadata_user;

-- For profiling and data quality (optional, only if needed)
-- Repeat for each schema where you want to run profiling/tests
GRANT SELECT ON ALL TABLES IN SCHEMA schema1 TO umetadata_user;
GRANT SELECT ON ALL TABLES IN SCHEMA schema2 TO umetadata_user;
