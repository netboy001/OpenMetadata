...
3. Click **Add API Key** / **Generate Token**, give it a descriptive name (e.g. `umetadata-ingestion`), and copy the value.
...
- **auto** (default): UMetadata tries `v2` first (Airflow 3.x), then falls back to `v1` (Airflow 2.x).
...
Database of the data source. This is an optional parameter, if you would like to restrict the metadata reading to a single database. When left blank, the UMetadata Ingestion attempts to scan all the databases.
$$
