...
- **Why it matters:** This is the server UMetadata will connect to for authentication.
- **Note:**
...
- **Why it matters:** UMetadata uses this account to search for and authenticate users.
...
- **Why it matters:** Required for UMetadata to authenticate as the admin user.
...
- **Default:** false
- **Example:** true
- **Why it matters:** Encrypts communication between UMetadata and LDAP server.
- **Note:**
...
- **Why it matters:** Tells UMetadata where to search for user accounts.
- **Note:** Should contain all users who need access to UMetadata
$$
...
- **Example:** mail
- **Why it matters:** UMetadata searches LDAP for users by email and uses this attribute as the primary identifier.
- **Critical:**
  - This is the most important LDAP field - if wrong, authentication will fail
  - The email address determines the UMetadata username
...
- **Validation:** UMetadata verifies this attribute exists on actual users before saving
$$
...
- **Validation:** UMetadata verifies groups can be found with this filter
$$
...
- **Validation:** UMetadata tests that groups exist with this combination
$$
...
- **Example:** member
- **Why it matters:** UMetadata checks if a user is a member of a group by looking for the user's DN in this attribute.
...
- **Validation:** UMetadata checks this attribute exists on actual group objects
$$

$$section
## Auth Roles Mapping $(id="authRolesMapping")

- **Definition:** Mapping between LDAP groups and UMetadata roles.
- **Example:** Map "cn=admins,ou=groups,dc=company,dc=com" to "Admin" role
- **Why it matters:** Automatically assigns UMetadata roles based on LDAP group membership.
- **Note:**
  - Use full LDAP Group Distinguished Names (DN) as keys
  - Map to existing UMetadata role names
  - Users in mapped LDAP groups will automatically receive the corresponding roles
  - Validation ensures all mapped roles exist in UMetadata
$$
...
The following settings control authorization and access control across UMetadata. These settings apply globally to all authentication providers.
$$

$$section
### Admin Principals $(id="adminPrincipals")

- **Definition:** List of user principals who will have admin access to UMetadata.
- **Example:** ["john.doe", "jane.admin", "admin"]
- **Why it matters:** These users will have full administrative privileges in UMetadata.
- **Note:**
...
- **Note:** When enabled, only users from the configured principal domain can access UMetadata
$$

$$section
### Allowed Domains $(id="allowedDomains")

- **Definition:** List of email domains that are permitted to access UMetadata.
...
- **Note:**
  ...
  - When `enforcePrincipalDomain` is enabled, only users with email addresses from these domains can access UMetadata
