...
```SQL
CREATE USER umetadata PASSWORD Password123;
```
...
```SQL
ALTER USER umetadata DISABLE PASSWORD LIFETIME;
```
...
You can find further information on the SAP Hana connector in the <a href="https://docs.wondersgroup.com/connectors/database/sap-hana" target="_blank">docs</a>.
...
If you are running the UMetadata ingestion in a docker and your services are hosted on the `localhost`, then use `host.docker.internal:39041` as the value.
$$
...
This is an optional parameter. When set, the value will be used to restrict the metadata reading to a single schema (corresponding to the value passed in this field). When left blank, UMetadata will scan all the schemas.
$$
