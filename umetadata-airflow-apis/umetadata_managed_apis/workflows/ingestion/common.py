...
from umetadata_managed_apis.api.utils import clean_dag_id
...
from umetadata_managed_apis.utils.airflow_version import is_airflow_3_or_higher
from umetadata_managed_apis.utils.logger import set_operator_logger, workflow_logger
from umetadata_managed_apis.utils.parser import (
...
class GetServiceException(Exception):
    ...
    def __init__(self, service_type: str, service_name: str):
        self.message = (
            ...
            " UMetadata client running in the Airflow host had issues getting"
...
class ClientInitializationError(Exception):
    """
    Exception to be thrown when couldn't initialize the Umetadata Client
    """
...
def build_source(ingestion_pipeline: IngestionPipeline) -> WorkflowSource:
    ...
    try:
        metadata = UMetadata(config=ingestion_pipeline.uMetadataServerConnection)

        # check we can access OM server
        metadata.health_check()
    except Exception as exc:
        raise ClientInitializationError(
            f"Failed to initialize the UMetadata client due to: {exc}."
            " Make sure that the Airflow host can reach the UMetadata"
            f" server running at {ingestion_pipeline.uMetadataServerConnection.hostPort}"
...
def execute_workflow(
    workflow: BaseWorkflow, workflow_config: UMetadataWorkflowConfig
) -> None:
    """
...
def metadata_ingestion_workflow(workflow_config: UMetadataWorkflowConfig):
    """
...
def build_workflow_config_property(
    ...
    """
    Prepare the workflow config with logLevels and uMetadataServerConfig
    ...
    return WorkflowConfig(
        ...
        uMetadataServerConfig=ingestion_pipeline.uMetadataServerConnection,
    )
...
def send_failed_status_callback(workflow_config: UMetadataWorkflowConfig, *_, **__):
    """
    ...
    try:
        ...
        metadata_config = workflow_config.workflowConfig.uMetadataServerConfig
        metadata = UMetadata(config=metadata_config)
...
def build_dag(
    task_name: str,
    ingestion_pipeline: IngestionPipeline,
    workflow_config: Union[UMetadataWorkflowConfig, UMetadataApplicationConfig],
    ...
    CustomPythonOperator(
        ...
        # each DAG will call its own UMetadataWorkflowConfig
