...
from umetadata_managed_apis.api.config import (
    AIRFLOW_DAGS_FOLDER,
    DAG_GENERATED_CONFIGS,
    PLUGIN_NAME,
)
from umetadata_managed_apis.api.response import ApiResponse
from umetadata_managed_apis.api.utils import (
    clean_dag_id,
    get_dagbag,
    import_path,
    scan_dags_job_background,
)
from umetadata_managed_apis.utils.logger import operations_logger
...
def dump_with_safe_jwt(ingestion_pipeline: IngestionPipeline) -> str:
    ...
    pipeline_json["uMetadataServerConnection"]["securityConfig"][
        "jwtToken"
    ] = ingestion_pipeline.uMetadataServerConnection.securityConfig.jwtToken.get_secret_value(
...
class DagDeployer:
    ...
    def __init__(self, ingestion_pipeline: IngestionPipeline):
        ...
        SecretsManagerFactory(
            ingestion_pipeline.uMetadataServerConnection.secretsManagerProvider,
            ingestion_pipeline.uMetadataServerConnection.secretsManagerLoader,
        )
