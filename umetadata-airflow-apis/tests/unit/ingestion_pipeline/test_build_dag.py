...
from umetadata_managed_apis.workflows.ingestion.common import build_dag
...
def _server_config() -> UMetadataConnection:
    return UMetadataConnection(
        hostPort="http://localhost:8585/api",
        authProvider="umetadata",
        securityConfig=UMetadataJWTClientConfig(jwtToken="x.y.z"),
    )
...
def _ingestion_pipeline(name: str) -> IngestionPipeline:
    return IngestionPipeline(
        ...
        uMetadataServerConnection=_server_config(),
        airflowConfig=AirflowConfig(),
        service=EntityReference(id=str(uuid.uuid4()), type="databaseService", name="svc"),
    )


def _workflow_config(name: str) -> UMetadataWorkflowConfig:
    return UMetadataWorkflowConfig(
        ...
        workflowConfig=WorkflowConfig(uMetadataServerConfig=_server_config()),
