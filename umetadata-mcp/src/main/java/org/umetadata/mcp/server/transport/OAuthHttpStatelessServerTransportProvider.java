package org.umetadata.mcp.server.transport;

import static org.umetadata.service.socket.SocketAddressFilter.validatePrefixedTokenRequest;
...
import org.umetadata.mcp.auth.OAuthClientMetadata;
import org.umetadata.mcp.auth.OAuthMetadata;
import org.umetadata.mcp.auth.OAuthToken;
import org.umetadata.mcp.auth.ProtectedResourceMetadata;
import org.umetadata.mcp.auth.RefreshToken;
import org.umetadata.mcp.auth.exception.TokenException;
import org.umetadata.mcp.server.auth.handlers.AuthorizationHandler;
import org.umetadata.mcp.server.auth.handlers.RegistrationHandler;
import org.umetadata.mcp.server.auth.handlers.RevocationHandler;
import org.umetadata.mcp.server.auth.middleware.ClientAuthenticator;
...
import org.umetadata.mcp.server.auth.util.ClientCredentialsExtractor;
import org.umetadata.mcp.server.auth.util.ClientCredentialsExtractor.InvalidClientCredentialsException;
...
public class OAuthHttpStatelessServerTransportProvider extends HttpServletStatelessServerTransport
  ...
  private volatile org.umetadata.mcp.server.auth.handlers.BasicAuthLoginServlet
  ...
  public void setBasicAuthLoginServlet(
      org.umetadata.mcp.server.auth.handlers.BasicAuthLoginServlet servlet) {
  ...
  public void onConfigurationChanged(
      org.umetadata.schema.api.security.AuthenticationConfiguration authConfig,
      org.umetadata.schema.api.security.AuthorizerConfiguration authzConfig,
  ...
  private void handleAuthorizeRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (authProvider instanceof org.umetadata.mcp.server.auth.provider.UserSSOOAuthProvider) {
      ...
      boolean isClientError =
          cause instanceof org.umetadata.mcp.auth.exception.AuthorizeException;
      response.setStatus(
          isClientError
  ...
  private void handleRegistrationRequest(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      org.umetadata.mcp.auth.exception.RegistrationException regEx = null;
      if (cause instanceof org.umetadata.mcp.auth.exception.RegistrationException r) {
        regEx = r;
      } else if (cause.getCause()
          instanceof org.umetadata.mcp.auth.exception.RegistrationException r) {
        regEx = r;
      }
