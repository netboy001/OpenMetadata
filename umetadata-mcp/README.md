# UMetadata MCP OAuth Implementation

OAuth 2.0 authentication server for Model Context Protocol (MCP) integration with UMetadata, enabling secure access to metadata through Claude Desktop and other MCP clients.

## Overview

This module implements a complete OAuth 2.0 Authorization Code Flow with PKCE for MCP clients, enabling **user authentication** via UMetadata's existing SSO providers (Google, Okta, Azure AD, Auth0, AWS Cognito, Custom OIDC, LDAP, SAML) or Basic Auth. The implementation provides secure, standards-compliant access to UMetadata's metadata management capabilities through MCP tools.

**Important**: This is **user SSO authentication** for MCP clients, not connector-based OAuth for data sources. Users authenticate with their UMetadata credentials (SSO or username/password), and MCP tools execute with that user's permissions.
...
- **Basic Auth** - Username/password authentication with UMetadata credentials
- **User Impersonation** - Support for impersonated user contexts in MCP tools
- **Auto-Detection** - Automatically selects SSO or Basic Auth based on UMetadata configuration
...
The OAuth server is configured in `umetadata.yaml`:
...
**Permission Model**: Tool permissions are enforced by UMetadata's Authorizer using the user's identity from the JWT. This ensures MCP users have the same access as they would in the UMetadata UI - respecting all policies, roles, and ownership rules. OAuth authenticates the user; the Authorizer enforces what they can access.
...
Tests SSO provider integration using pac4j with mock identity providers (Google, Okta, Azure AD, etc.). The UserSSOOAuthProvider auto-detects the configured SSO provider from UMetadata's authentication configuration.
