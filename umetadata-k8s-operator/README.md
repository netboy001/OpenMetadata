# UMetadata Kubernetes Operator

A Kubernetes operator that provides guaranteed exit handler execution for UMetadata pipeline jobs using a two-stage execution pattern similar to Argo Workflows' `onExit` handlers.
...
5. **Status Update**: Exit handler reports final status to UMetadata
...
java -jar target/umetadata-k8s-operator-1.12.0-SNAPSHOT-boot.jar
```
...
The operator is deployed as part of the UMetadata Helm chart:
...
1. **Deploy CRD**:
```bash
kubectl apply -f charts/umetadata/templates/omjob-crd.yaml
```

2. **Deploy RBAC**:
```bash
kubectl apply -f charts/umetadata/templates/omjob-operator-rbac.yaml
```

3. **Deploy Operator**:
```bash
kubectl apply -f charts/umetadata/templates/omjob-operator-deployment.yaml
```
...
metadata:
  ...
  namespace: umetadata-pipelines
spec:
  # Main ingestion pod specification
  mainPodSpec:
    image: "umetadata/ingestion-base:1.5.0"
    imagePullPolicy: "IfNotPresent"
    serviceAccountName: "umetadata-ingestion"
...
```
src/main/java/org/umetadata/operator/
