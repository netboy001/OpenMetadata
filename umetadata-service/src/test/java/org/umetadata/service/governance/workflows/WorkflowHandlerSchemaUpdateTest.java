...
class WorkflowHandlerSchemaUpdateTest {
  ...
  void runtimeModeWrapsFlowableWrongDbExceptionWithActionableMessage() {
    try (MockedConstruction<StandaloneProcessEngineConfiguration> engineMock =
      ...
      assertTrue(ex.getMessage().contains("umetadata-ops.sh migrate"));
  ...
  private UMetadataApplicationConfig buildMockConfig() {
    UMetadataApplicationConfig config = mock(UMetadataApplicationConfig.class);
    ...
    lenient().when(dsf.getUrl()).thenReturn("jdbc:postgresql://localhost:5432/umetadata_db");
    lenient().when(dsf.getUser()).thenReturn("umetadata_user");
    lenient().when(dsf.getPassword()).thenReturn("umetadata_password");
