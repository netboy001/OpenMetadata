...
public class MicrometerBundleTest {
  ...
  @Mock private UMetadataApplicationConfig config;
  ...
  public void testRun() {
    ...
    assertNotNull(bundle.getUMetadataMetrics());
    assertNotNull(bundle.getPrometheusMeterRegistry());
  ...
  @Test
  public void testUMetadataMetricsIntegration() {
    // Initialize and run bundle
    bundle.initialize(bootstrap);
    bundle.run(config, environment);

    // Get UMetadataMetrics instance
    UMetadataMetrics omMetrics = bundle.getUMetadataMetrics();
    ...
    assertTrue(
        !metrics.contains(
                "entity_operations_total{application=\"umetadata\",cluster=\"test-cluster\",operation=\"create\",type=\"test\"} 0.0")
            || metrics.contains(
                "entity_operations_total{application=\"umetadata\",cluster=\"test-cluster\",operation=\"create\",type=\"test\"} 1.0"));
  }
}
