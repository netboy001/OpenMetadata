...
import org.umetadata.schema.entity.teams.*;
import org.umetadata.schema.services.connections.database.*;
import org.umetadata.schema.services.connections.database.common.*;
...
import org.umetadata.service.cache.CacheKeys;
 ...
 * This test extends UMetadataApplicationTest to get a full application context.
 */
@Slf4j
public class CacheResourceIntegrationTest extends UMetadataApplicationTest {
  ...
  void testLiveCachingOnEntityCreation(TestInfo testInfo) throws Exception {
    ...
    // Note: UMetadata may use read-through caching, not write-through
    ...
    if (cachedBeforeDelete != null
      ...
      Response restoreResponse =
          SecurityUtil.addHeaders(getResource("tables/restore"), ADMIN_AUTH_HEADERS)
              .put(
                  Entity.json(
                      new org.umetadata.schema.api.data.RestoreEntity().withId(tableId)));
  ...
  void testTagsRelationshipCaching() throws Exception {
    ...
    String tagsCacheKey = cacheKeys.tags(org.umetadata.service.Entity.TABLE, table.getId());
