...
import static org.umetadata.schema.type.Relationship.OWNS;
...
import static org.umetadata.service.util.EntityUtil.taskMatch;
...
import org.umetadata.schema.type.PipelineExecutionTrend;
import org.umetadata.schema.type.PipelineExecutionTrendList;
import org.umetadata.schema.type.PipelineMetrics;
import org.umetadata.schema.type.PipelineObservabilityResponse;
import org.umetadata.schema.type.PipelineRuntimeTrend;
import org.umetadata.schema.type.PipelineRuntimeTrendList;
import org.umetadata.schema.type.PipelineSummary;
import org.umetadata.schema.type.Relationship;
import org.umetadata.schema.type.ServiceBreakdown;
import org.umetadata.schema.type.Status;
import org.umetadata.schema.type.TableObservabilityData;
import org.umetadata.schema.type.TagLabel;
import org.umetadata.schema.type.Task;
...
import org.umetadata.service.rdf.RdfRepository;
...
import org.umetadata.service.resources.pipelines.PipelineResource;
...
public class PipelineRepository extends EntityRepository<Pipeline> {
          ...
          pipelineStatuses.stream()
              .map(
                  ps -> {
                    ...
                    List<org.umetadata.schema.type.Status> filteredTasks =
  ...
  public PipelineObservabilityResponse getPipelineObservability(
    ...
    try {
      ...
      for (Map.Entry<UUID, List<CollectionDAO.ExtensionWithIdAndSchemaObject>> entry :
        ...
        try {
          ...
          List<org.umetadata.schema.type.PipelineObservability> observabilityData =
              new ArrayList<>();
          for (CollectionDAO.ExtensionWithIdAndSchemaObject record : tableRecords) {
            if (record.getExtension().equals(extensionKey)) {
              org.umetadata.schema.type.PipelineObservability observability =
                  JsonUtils.readValue(
                      record.getJson(), org.umetadata.schema.type.PipelineObservability.class);
    ...
    return new PipelineObservabilityResponse()
        ...
        .withPaging(
            new org.umetadata.schema.type.Paging()
    ...
    try {
      ...
      for (PipelineStatus status : statuses) {
        ...
        if (status.getTaskStatus() != null && !status.getTaskStatus().isEmpty()) {
          ...
          for (org.umetadata.schema.type.Status task : status.getTaskStatus()) {
