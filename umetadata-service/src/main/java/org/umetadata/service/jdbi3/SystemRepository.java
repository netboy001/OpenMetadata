...
import static org.umetadata.service.apps.bundles.insights.DataInsightsApp.getDataStreamName;
...
import org.umetadata.api.configuration.UiThemePreference;
import org.umetadata.catalog.security.client.SamlSSOClientConfig;
import org.umetadata.schema.api.configuration.LogStorageConfiguration;
...
import org.umetadata.schema.auth.LdapConfiguration;
import org.umetadata.schema.configuration.AssetCertificationSettings;
import org.umetadata.schema.configuration.ExecutorConfiguration;
import org.umetadata.schema.configuration.HistoryCleanUpConfiguration;
import org.umetadata.schema.configuration.SecurityConfiguration;
import org.umetadata.schema.configuration.WorkflowSettings;
...
import org.umetadata.schema.security.client.OidcClientConfig;
...
import org.umetadata.schema.service.configuration.slackApp.SlackAppConfiguration;
...
import org.umetadata.schema.system.FieldError;
import org.umetadata.schema.system.SecurityValidationResponse;
import org.umetadata.schema.system.StepValidation;
import org.umetadata.schema.system.ValidationResponse;
import org.umetadata.schema.util.EntitiesCount;
import org.umetadata.schema.util.ServicesCount;
...
import org.umetadata.service.jdbi3.CollectionDAO.SystemDAO;
import org.umetadata.service.logstorage.LogStorageFactory;
import org.umetadata.service.logstorage.LogStorageInterface;
...
import org.umetadata.service.secrets.masker.PasswordEntityMasker;
...
import org.umetadata.service.security.auth.LoginAttemptCache;
import org.umetadata.service.security.auth.validator.Auth0Validator;
import org.umetadata.service.security.auth.validator.AzureAuthValidator;
import org.umetadata.service.security.auth.validator.CognitoAuthValidator;
import org.umetadata.service.security.auth.validator.CustomOidcValidator;
import org.umetadata.service.security.auth.validator.GoogleAuthValidator;
import org.umetadata.service.security.auth.validator.OidcDiscoveryValidator;
import org.umetadata.service.security.auth.validator.OktaAuthValidator;
import org.umetadata.service.security.auth.validator.SamlValidator;
import org.umetadata.service.util.EntityUtil;
import org.umetadata.service.util.LdapUtil;
import org.umetadata.service.util.UMetadataConnectionBuilder;
import org.umetadata.service.util.RestUtil;
import org.umetadata.service.util.ValidationErrorBuilder;
import org.umetadata.service.util.ValidationErrorBuilder.FieldPaths;
...
public class SystemRepository {
  ...
  public Settings getOMBaseUrlConfigInternal() {
    try {
      Settings setting =
          dao.getConfigWithKey(SettingsType.U_METADATA_BASE_URL_CONFIGURATION.value());
      ...
      LOG.error(
          "Error while trying to fetch UMetadataBaseUrlConfiguration Settings {}",
          ex.getMessage());
    }
    return null;
  }
  ...
  public void updateSetting(Settings setting) {
    try {
      ...
      } else if (setting.getConfigType() == SettingsType.U_METADATA_BASE_URL_CONFIGURATION) {
        UMetadataBaseUrlConfiguration omBaseUrl =
            JsonUtils.convertValue(
                setting.getConfigValue(), UMetadataBaseUrlConfiguration.class);
  ...
  public ValidationResponse validateSystem(
      UMetadataApplicationConfig applicationConfig,
  ...
  public void addExtraValidations(
      UMetadataApplicationConfig applicationConfig, ValidationResponse validation) {}

  private StepValidation getEmbeddingsValidation(UMetadataApplicationConfig applicationConfig) {
  ...
  private StepValidation validateEmbeddingGeneration(
    ...
    String testText = "UMetadata embedding validation test";
  ...
  private String getEmbeddingConfigurationMessage(UMetadataApplicationConfig applicationConfig) {
    try {
  ...
  private StepValidation getDatabaseValidation(UMetadataApplicationConfig applicationConfig) {
    try {
      dao.testConnection();
  ...
  private StepValidation getSearchValidation(UMetadataApplicationConfig applicationConfig) {
    SearchRepository searchRepository = Entity.getSearchRepository();
  ...
  List<String> findMissingIndexes(SearchRepository searchRepository) {
    ...
    try {
      Map<String, org.umetadata.search.IndexMapping> indexMap =
          searchRepository.getEntityIndexMap();
      for (Map.Entry<String, org.umetadata.search.IndexMapping> entry : indexMap.entrySet()) {
  ...
  private StepValidation getJWKsValidation(
      UMetadataApplicationConfig applicationConfig, JwtFilter jwtFilter) {
    SecretsManager secretsManager = SecretsManagerFactory.getSecretsManager();
    UMetadataConnection uMetadataServerConnection =
        new UMetadataConnectionBuilder(applicationConfig).build();
    UMetadataJWTClientConfig realJWTConfig =
        secretsManager.decryptJWTConfig(uMetadataServerConnection.getSecurityConfig());
    try {
    ...
    try {
      ...
      if (!nullOrEmpty(ldapConfig.getAuthRolesMapping())) {
        try {
          ...
          // Validate that mapped roles exist in UMetadata
          ...
          if (!invalidRoles.isEmpty()) {
            return ValidationErrorBuilder.createFieldError(
                ...
                "The following roles do not exist in UMetadata: "
  ...
  private FieldError validateSamlConfiguration(
      SamlSSOClientConfig samlConfig, UMetadataApplicationConfig applicationConfig) {
    try {
  ...
  private StepValidation getLogStorageValidation(UMetadataApplicationConfig applicationConfig) {
    try {
