...
import org.umetadata.service.jdbi3.MetadataServiceRepository;
import org.umetadata.service.limits.Limits;
import org.umetadata.service.resources.Collection;
import org.umetadata.service.resources.services.ServiceEntityResource;
...
@Tag(
    ...
    description =
        "APIs related to creating and managing other Metadata Services that "
            + "UMetadata integrates with such as `Apache Atlas`, `Amundsen`, etc.")
...
public class MetadataServiceResource
  ...
  public static final String UMETADATA_SERVICE = "UMetadata";
  ...
  private void registerMetadataServices(UMetadataApplicationConfig config) throws IOException {
    List<MetadataService> servicesList =
        repository.getEntitiesFromSeedData(".*json/data/metadataService/UmetadataService.json$");
    if (!nullOrEmpty(servicesList)) {
      MetadataService uMetadataService = servicesList.get(0);
      uMetadataService.setId(UUID.randomUUID());
      uMetadataService.setUpdatedBy(ADMIN_USER_NAME);
      uMetadataService.setUpdatedAt(System.currentTimeMillis());
      if (config.getElasticSearchConfiguration() != null) {
        UMetadataConnection uMetadataServerConnection =
            new UMetadataConnectionBuilder(config)
                .build()
                .withElasticsSearch(
                    getElasticSearchConnectionSink(config.getElasticSearchConfiguration()));
        MetadataConnection metadataConnection =
            new MetadataConnection().withConfig(uMetadataServerConnection);
        uMetadataService.setConnection(metadataConnection);
      } else {
        LOG.error("[MetadataService] Missing Elastic Search Config.");
      }
      repository.setFullyQualifiedName(uMetadataService);
      repository.createOrUpdate(null, uMetadataService, ADMIN_USER_NAME);
    } else {
      throw new IOException("Failed to initialize UMetadata Service.");
    }
  }
