...
import org.umetadata.service.util.SchemaFieldExtractor;

@Path("/v1/metadata/types")
@Tag(
    name = "Metadata",
    description =
        "These APIs are for managing custom property definitions in UMetadata. Use these APIs to "
            ...
            + "only custom properties using existing UMetadata data types.")
...
public class TypeResource extends EntityResource<Type, TypeRepository> {
  ...
  public void initialize(UMetadataApplicationConfig config) {
    // Load types defined in UMetadata schemas
  ...
  @Operation(
      ...
      description =
          "Create a new custom property definition that can be applied to entities. "
              + "This creates a property template using existing UMetadata data types "
