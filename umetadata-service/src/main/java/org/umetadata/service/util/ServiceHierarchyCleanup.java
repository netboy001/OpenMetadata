 ...
 * This class provides common functionality that can be used by both UMetadataOperations
...
public class ServiceHierarchyCleanup {
  ...
  /**
   * Performs comprehensive service hierarchy cleanup for all UMetadata service types.
   *
  ...
  /**
   * Defines all service hierarchies in UMetadata.
  ...
  public void printCleanupResults(HierarchyCleanupResult result) {
    ...
    if (!rows.isEmpty()) {
      UMetadataOperations.printToAsciiTable(columns, rows, "No broken entities found");
    }
  }
}
