...
import static org.umetadata.service.Entity.ADMIN_ROLE;
...
import org.umetadata.sdk.exception.UserCreationException;
...
public final class UserUtil {
  ...
  public static boolean assignTeamsFromClaim(User user, List<String> teamNames) {
    ...
    for (String teamName : teamNames) {
      ...
      try {
        ...
        LOG.warn(
            "Team '{}' from claim mapping not found in UMetadata. "
                + "User '{}' will not be assigned to this team.",
            teamName,
            user.getName());
      } catch (Exception e) {
   ...
   *   <li>Otherwise:
   *       <ul>
   *         <li>If airflow configuration is 'umetadata' and server auth provider is not basic, add JWT auth
  ...
  private static JWTAuthMechanism buildJWTAuthMechanism(
      UMetadataJWTClientConfig jwtClientConfig, User user) {
