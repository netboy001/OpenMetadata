package org.umetadata.service.monitoring;
 ...
 */
@Slf4j
public class MicrometerBundle implements ConfiguredBundle<UMetadataApplicationConfig> {
  private PrometheusMeterRegistry prometheusMeterRegistry;
  private UMetadataMetrics uMetadataMetrics;
  ...
  @Override
  public void run(UMetadataApplicationConfig configuration, Environment environment) {
    // Configure common tags
    String clusterName =
        configuration.getClusterName() != null ? configuration.getClusterName() : "default";
    prometheusMeterRegistry
        .config()
        .commonTags("application", "umetadata", "cluster", clusterName);

    // Bind JVM and system metrics
    bindSystemMetrics();

    // Create UMetadataMetrics instance
    uMetadataMetrics = new UMetadataMetrics(prometheusMeterRegistry);
    ...
    environment.jersey().register(new MetricsRequestFilter(uMetadataMetrics));
        ...
        .register(
            new AbstractBinder() {
              ...
              protected void configure() {
                ...
                bind(uMetadataMetrics).to(UMetadataMetrics.class);
  ...
  public UMetadataMetrics getUMetadataMetrics() {
    return uMetadataMetrics;
  }
