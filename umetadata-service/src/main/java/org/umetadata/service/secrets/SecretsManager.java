...
package org.umetadata.service.secrets;
...
import org.umetadata.annotations.PasswordField;
...
import org.umetadata.schema.security.secrets.Parameters;
...
import org.umetadata.service.exception.SecretsManagerException;
import org.umetadata.service.fernet.Fernet;
import org.umetadata.service.secrets.converter.ClassConverterFactory;
import org.umetadata.service.util.AuthenticationMechanismBuilder;
import org.umetadata.service.util.IngestionPipelineBuilder;
import org.umetadata.service.util.ReflectionUtil;
...
public abstract class SecretsManager {
  ...
  private static final Set<Class<?>> DO_NOT_ENCRYPT_CLASSES =
      Set.of(UMetadataJWTClientConfig.class, BasicAuthMechanism.class);
  ...
  public UMetadataJWTClientConfig decryptJWTConfig(UMetadataJWTClientConfig jwtConfig) {
    if (jwtConfig != null) {
      try {
        UMetadataJWTClientConfig decrypted =
            (UMetadataJWTClientConfig) decryptPasswordFields(jwtConfig);
        return (UMetadataJWTClientConfig) getSecretFields(decrypted);
      } catch (Exception e) {
        throw new SecretsManagerException(
  ...
  public UMetadataConnection encryptUMetadataConnection(
      UMetadataConnection uMetadataConnection, boolean store) {
    if (uMetadataConnection != null) {
      UMetadataConnection uMetadataConnectionConverted =
          (UMetadataConnection)
              ClassConverterFactory.getConverter(UMetadataConnection.class)
                  .convert(uMetadataConnection);
      try {
        encryptPasswordFields(
            uMetadataConnectionConverted, buildSecretId(true, "serverconnection"), store);
      } catch (Exception e) {
        throw new SecretsManagerException(
            Response.Status.BAD_REQUEST, "Failed to encrypt UMetadataConnection instance.");
      }
      return uMetadataConnectionConverted;
    }
    return null;
  }

  public UMetadataConnection decryptUMetadataConnection(
      UMetadataConnection uMetadataConnection) {
    if (uMetadataConnection != null) {
      UMetadataConnection uMetadataConnectionConverted =
          (UMetadataConnection)
              ClassConverterFactory.getConverter(UMetadataConnection.class)
                  .convert(uMetadataConnection);
      try {
        decryptPasswordFields(uMetadataConnectionConverted);
      } catch (Exception e) {
        throw new SecretsManagerException(
            Response.Status.BAD_REQUEST, "Failed to decrypt UMetadataConnection instance.");
      }
      return uMetadataConnectionConverted;
    }
    return null;
        ...
        Arrays.stream(toEncryptObject.getClass().getMethods())
            ...
            .forEach(
                method -> {
                  ...
                  // if the object matches the package of umetadata
                  if (CommonUtil.isUMetadataObject(obj)) {
                    // encryptPasswordFields
                    encryptPasswordFields(
                        obj,
