...
import static org.umetadata.service.security.JwtFilter.BOT_CLAIM;
import static org.umetadata.service.security.JwtFilter.EMAIL_CLAIM_KEY;
import static org.umetadata.service.security.JwtFilter.USERNAME_CLAIM_KEY;
...
public final class SecurityUtil {
  ...
  public static String getDomain(UMetadataApplicationConfig config) {
  ...
  /**
   * Returns true only when the token was provably minted by UMetadata itself. A token qualifies
   * when its key id matches UMetadata's own signing key id and its issuer matches UMetadata's
   * configured issuer. The key id is the trust anchor: the signature has already been verified
   * against the public key served for that key id, so a matching key id proves UMetadata's
   ...
   */
  public static boolean isUMetadataIssuedToken(
      Map<String, Claim> claims,
      String tokenKeyId,
      String uMetadataIssuer,
      String uMetadataKeyId) {
    boolean issuedByUMetadata = false;
    if (!nullOrEmpty(uMetadataIssuer) && !nullOrEmpty(uMetadataKeyId)) {
      boolean keyIdMatches = uMetadataKeyId.equals(tokenKeyId);
      boolean issuerMatches = uMetadataIssuer.equals(getClaimOrObject(claims.get(ISSUER_CLAIM)));
      issuedByUMetadata = keyIdMatches && issuerMatches;
    }
    return issuedByUMetadata;
  }
}
