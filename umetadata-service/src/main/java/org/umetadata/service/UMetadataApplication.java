...
import static org.umetadata.service.util.jdbi.JdbiUtils.createAndSetupJDBI;
...
import org.umetadata.search.IndexMappingLoader;
...
import org.umetadata.service.apps.bundles.searchIndex.distributed.DistributedJobParticipant;
import org.umetadata.service.apps.bundles.searchIndex.distributed.ServerIdentityResolver;
import org.umetadata.service.apps.scheduler.AppScheduler;
import org.umetadata.service.audit.AuditLogEventPublisher;
import org.umetadata.service.audit.AuditLogRepository;
import org.umetadata.service.cache.CacheConfig;
import org.umetadata.service.config.CacheConfiguration;
import org.umetadata.service.config.OMWebBundle;
import org.umetadata.service.config.OMWebConfiguration;
import org.umetadata.service.events.EventFilter;
import org.umetadata.service.events.EventPubSub;
import org.umetadata.service.events.scheduled.EventSubscriptionScheduler;
import org.umetadata.service.events.scheduled.ServicesStatusJobHandler;
import org.umetadata.service.exception.CatalogGenericExceptionMapper;
import org.umetadata.service.exception.ConstraintViolationExceptionMapper;
import org.umetadata.service.exception.JsonMappingExceptionMapper;
import org.umetadata.service.exception.OMErrorPageHandler;
import org.umetadata.service.fernet.Fernet;
import org.umetadata.service.governance.workflows.WorkflowHandler;
import org.umetadata.service.jdbi3.BulkExecutor;
import org.umetadata.service.jdbi3.CollectionDAO;
import org.umetadata.service.jdbi3.EntityRelationshipRepository;
...
import org.umetadata.service.jdbi3.locator.ConnectionAwareAnnotationSqlLocator;
import org.umetadata.service.jdbi3.locator.ConnectionType;
import org.umetadata.service.jobs.EnumCleanupHandler;
import org.umetadata.service.jobs.GenericBackgroundWorker;
import org.umetadata.service.jobs.JobDAO;
import org.umetadata.service.jobs.JobHandlerRegistry;
import org.umetadata.service.limits.DefaultLimits;
import org.umetadata.service.limits.Limits;
import org.umetadata.service.migration.MigrationValidationClient;
import org.umetadata.service.migration.api.MigrationWorkflow;
import org.umetadata.service.monitoring.EventMonitor;
import org.umetadata.service.monitoring.EventMonitorConfiguration;
import org.umetadata.service.monitoring.EventMonitorFactory;
import org.umetadata.service.monitoring.EventMonitorPublisher;
import org.umetadata.service.monitoring.JettyMetricsIntegration;
import org.umetadata.service.monitoring.UserMetricsServlet;
import org.umetadata.service.rdf.RdfUpdater;
import org.umetadata.service.resources.CollectionRegistry;
import org.umetadata.service.resources.audit.AuditLogResource;
import org.umetadata.service.resources.databases.DatasourceConfig;
import org.umetadata.service.resources.filters.ETagRequestFilter;
import org.umetadata.service.resources.filters.ETagResponseFilter;
import org.umetadata.service.resources.settings.SettingsCache;
import org.umetadata.service.resources.system.DiagnosticsResource;
import org.umetadata.service.search.SearchRepository;
import org.umetadata.service.search.SearchRepositoryFactory;
import org.umetadata.service.search.opensearch.OpenSearchSearchManager;
...
import org.umetadata.service.security.AuthCallbackServlet;
import org.umetadata.service.security.AuthLoginServlet;
import org.umetadata.service.security.AuthLogoutServlet;
import org.umetadata.service.security.AuthRefreshServlet;
import org.umetadata.service.security.AuthServeletHandlerFactory;
import org.umetadata.service.security.AuthServeletHandlerRegistry;
import org.umetadata.service.security.AuthenticationCodeFlowHandler;
import org.umetadata.service.security.Authorizer;
import org.umetadata.service.security.ContainerRequestFilterManager;
import org.umetadata.service.security.CspNonceHandler;
import org.umetadata.service.security.DelegatingContainerRequestFilter;
import org.umetadata.service.security.NoopAuthorizer;
import org.umetadata.service.security.NoopFilter;
import org.umetadata.service.security.auth.AuthenticatorHandler;
import org.umetadata.service.security.auth.BasicAuthenticator;
import org.umetadata.service.security.auth.LdapAuthenticator;
import org.umetadata.service.security.auth.NoopAuthenticator;
import org.umetadata.service.security.auth.SecurityConfigurationManager;
import org.umetadata.service.security.auth.UserActivityFilter;
import org.umetadata.service.security.auth.UserActivityTracker;
import org.umetadata.service.security.jwt.JWTTokenGenerator;
import org.umetadata.service.security.policyevaluator.SubjectCache;
import org.umetadata.service.security.saml.OMMicrometerHttpFilter;
import org.umetadata.service.security.saml.SamlAssertionConsumerServlet;
import org.umetadata.service.security.saml.SamlLoginServlet;
import org.umetadata.service.security.saml.SamlLogoutServlet;
import org.umetadata.service.security.saml.SamlMetadataServlet;
import org.umetadata.service.security.saml.SamlSettingsHolder;
import org.umetadata.service.security.saml.SamlTokenRefreshServlet;
import org.umetadata.service.socket.FeedServlet;
import org.umetadata.service.socket.Jetty12WebSocketHandler;
import org.umetadata.service.socket.UMetadataAssetServlet;
import org.umetadata.service.socket.SocketAddressFilter;
import org.umetadata.service.socket.WebSocketManager;
import org.umetadata.service.swagger.SwaggerBundle;
import org.umetadata.service.swagger.SwaggerBundleConfiguration;
import org.umetadata.service.util.AsyncService;
import org.umetadata.service.util.CustomParameterNameProvider;
import org.umetadata.service.util.incidentSeverityClassifier.IncidentSeverityClassifierInterface;
...
@OpenAPIDefinition(
    info =
        @Info(
            title = "UMetadata APIs",
            version = "1.12.14",
            description = "Common types and API definition for UMetadata",
            contact =
                @Contact(
                    name = "UMetadata",
...
@SecurityScheme(
    ...
    scheme = "bearer",
    bearerFormat = "JWT")
public class UMetadataApplication extends Application<UMetadataApplicationConfig> {
  ...
  @Override
  public void run(UMetadataApplicationConfig catalogConfig, Environment environment)
    ...
    UMetadataApplicationConfigHolder.initialize(catalogConfig);
    ...
    // Rebuild caches with configured limits (cacheMemory section in umetadata.yaml)
  ...
  protected void registerMCPServer(
      UMetadataApplicationConfig catalogConfig, Environment environment) {
    ...
    try {
      if (ApplicationContext.getInstance().getAppIfExists("McpApplication") != null) {
        Class<?> mcpServerClass = Class.forName("org.umetadata.mcp.McpServer");
  ...
  private void registerHealthCheckJobs(UMetadataApplicationConfig catalogConfig) {
  ...
  private void registerAuthServlets(UMetadataApplicationConfig config, Environment environment) {
  ...
  public static boolean isHttps(UMetadataApplicationConfig configuration) {
   ...
   * rejects special characters that were allowed in Jetty 11. UMetadata allows special
   ...
   */
  private void configureUriCompliance(UMetadataApplicationConfig configuration) {
    if (configuration.getServerFactory() instanceof DefaultServerFactory serverFactory) {
   ...
   *
   * This is required because UMetadata entity names can contain special characters
   ...
   */
  protected void initializeCoreSearchInfrastructure(UMetadataApplicationConfig config) {
  ...
  private void registerHealthCheck(Environment environment) {
    ...
    // any downstream system. See UMetadataServerHealthCheck for the design rationale.
    environment
        .healthChecks()
        .register("UMetadataServerHealthCheck", new UMetadataServerHealthCheck());
  }
  ...
  private void registerAssetServlet(
    ...
    UMetadataAssetServlet assetServlet =
        new UMetadataAssetServlet(
  ...
  @SneakyThrows
  @Override
  public void initialize(Bootstrap<UMetadataApplicationConfig> bootstrap) {
    ...
    bootstrap
        .getObjectMapper()
        .registerSubtypes(
            org.umetadata.service.events.AuditOnlyFilterFactory.class,
            org.umetadata.service.events.AuditExcludeFilterFactory.class);

    bootstrap.addBundle(
        new SwaggerBundle<>() {
          @Override
          protected SwaggerBundleConfiguration getSwaggerBundleConfiguration(
              UMetadataApplicationConfig catalogConfig) {
    ...
    bootstrap.addBundle(
        new OMWebBundle<>() {
          ...
          public OMWebConfiguration getWebConfiguration(
              final UMetadataApplicationConfig configuration) {
            return configuration.getWebConfiguration();
          }
        });

    // Add Micrometer bundle for Prometheus metrics
    bootstrap.addBundle(new org.umetadata.service.monitoring.MicrometerBundle());

    // Add Cache bundle for Redis/cache support
    bootstrap.addBundle(new org.umetadata.service.cache.CacheBundle());

    super.initialize(bootstrap);
  }

  private void validateMigrations(Jdbi jdbi, UMetadataApplicationConfig conf)
      throws IOException {
  ...
  private void validateConfiguration(UMetadataApplicationConfig catalogConfig)
    ...
    if (catalogConfig.getAuthorizerConfiguration().getBotPrincipals() != null) {
      throw new ConfigurationException(
          "'botPrincipals' configuration is deprecated. Please remove it from "
              + "'umetadata.yaml and restart the server");
    }
  ...
  public void reinitializeAuthSystem(
      UMetadataApplicationConfig config, Environment environment) {
    try {
  ...
  private void registerLimits(UMetadataApplicationConfig serverConfig)
      throws NoSuchMethodException,
          ClassNotFoundException,
          IllegalAccessException,
          InvocationTargetException,
  ...
  private void registerEventFilter(
    ...
    environment.jersey().register(org.umetadata.service.monitoring.MetricsRequestFilter.class);
  }
  ...
  private void registerEventPublisher(UMetadataApplicationConfig uMetadataApplicationConfig) {

    EventPubSub.addEventHandler(new AuditLogEventPublisher(auditLogRepository));

    if (uMetadataApplicationConfig.getEventMonitorConfiguration() != null) {
      final EventMonitor eventMonitor =
          EventMonitorFactory.createEventMonitor(
              uMetadataApplicationConfig.getEventMonitorConfiguration(),
              uMetadataApplicationConfig.getClusterName());
      EventMonitorPublisher eventMonitorPublisher =
          new EventMonitorPublisher(
              uMetadataApplicationConfig.getEventMonitorConfiguration(), eventMonitor);
      EventPubSub.addEventHandler(eventMonitorPublisher);
    }
  }

  private void registerResources(
      UMetadataApplicationConfig config, Environment environment, Jdbi jdbi) {
  ...
  public static void main(String[] args) throws Exception {
    UMetadataApplication uMetadataApplication = new UMetadataApplication();
    uMetadataApplication.run(args);
  }
