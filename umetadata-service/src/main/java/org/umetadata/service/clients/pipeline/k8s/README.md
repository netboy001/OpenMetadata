...
the `umetadata-k8s-operator` custom resources (OMJob/CronOMJob) execute them.
...
1. Build workflow config (`WorkflowConfigBuilder`).
   - Ensures UMetadata server connection exists.
...
- `app.kubernetes.io/managed-by = umetadata`
