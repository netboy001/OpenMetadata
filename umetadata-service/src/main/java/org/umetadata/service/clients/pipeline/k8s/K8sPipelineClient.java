...
package org.umetadata.service.clients.pipeline.k8s;
...
import org.umetadata.sdk.exception.PipelineServiceClientException;
import org.umetadata.service.clients.pipeline.PipelineServiceClient;
import org.umetadata.service.clients.pipeline.config.WorkflowConfigBuilder;
import org.umetadata.service.exception.IngestionPipelineDeploymentException;
...
public class K8sPipelineClient extends PipelineServiceClient {
  ...
  private static final String LABEL_VALUE_UMETADATA = "umetadata";
  ...
  private static final String PIPELINE_MISSING_CONNECTION_WARNING =
      "Pipeline %s missing UMetadataServerConnection - creating default configuration from client config";
  private static final String PIPELINE_MISSING_SECURITY_CONFIG_ERROR =
      "Pipeline %s has UMetadataServerConnection but missing securityConfig. The JWT token and authentication config are required for ingestion to work.";
  private static final String PIPELINE_PROPER_CONNECTION_DEBUG =
      "Pipeline %s has proper UMetadataServerConnection with security config";
  ...
  private static final String PIPELINE_CONNECTION_STATE_ERROR =
      "Pipeline UMetadataServerConnection.securityConfig is required but not set. This indicates the JWT token or authentication configuration is missing.";
  ...
  private static final String DEFAULT_CONNECTION_INFO =
      "Set default UMetadataServerConnection for pipeline %s with endpoint %s";
  ...
  private String buildWorkflowConfigWithOverrides(
      ...
      Map<String, Object> configOverride) {
    try {
      // Ensure the pipeline has proper UMetadataServerConnection configured
    ...
    return new V1Job()
        .metadata(
            new V1ObjectMeta()
                ...
                .labels(
                    Map.of(
                        LABEL_APP, LABEL_VALUE_UMETADATA,
                        LABEL_COMPONENT, LABEL_VALUE_AUTOMATION,
                        LABEL_MANAGED_BY, LABEL_VALUE_UMETADATA,
                        LABEL_RUN_ID, runId)))
        .spec(
            new V1JobSpec()
                .backoffLimit(k8sConfig.getBackoffLimit())
  ...
  /**
   * Ensures that the IngestionPipeline has a properly configured UMetadataServerConnection
  ...
  private void ensureServerConnectionConfigured(IngestionPipeline pipeline) {
    if (pipeline.getuMetadataServerConnection() == null) {
      ...
      // This ensures the pipeline can authenticate with the UMetadata server
  ...
  /**
   * Creates a default UMetadataConnection based on the client configuration.
   ...
   *
   * @return A properly configured UMetadataConnection with ingestion bot authentication
   */
  private org.umetadata.schema.services.connections.metadata.UMetadataConnection
    ...
    return new org.umetadata.schema.services.connections.metadata.UMetadataConnection()
        .withHostPort(metadataApiEndpoint)
        .withAuthProvider(
            org.umetadata.schema.services.connections.metadata.AuthProvider.UMETADATA)
        .withSecurityConfig(
            new org.umetadata.schema.security.client.UMetadataJWTClientConfig()
                .withJwtToken(jwtToken))
        .withVerifySSL(org.umetadata.schema.security.ssl.VerifySSL.NO_SSL)
        .withSecretsManagerLoader(
            org.umetadata.schema.security.secrets.SecretsManagerClientLoader.NOOP)
        .withSecretsManagerProvider(
            org.umetadata.schema.security.secrets.SecretsManagerProvider.DB);
  }

  /**
   * Retrieves the JWT token for the ingestion-bot user.
   * This follows the same pattern as UMetadataOperations.getIngestionBotToken().
   *
   * @return The JWT token string, or null if not available
   */
  private String getIngestionBotToken() {
    try {
      // Use the same pattern as UMetadataOperations
      org.umetadata.service.jdbi3.BotRepository botRepository =
          (org.umetadata.service.jdbi3.BotRepository)
              org.umetadata.service.Entity.getEntityRepository(
                  org.umetadata.service.Entity.BOT);
      org.umetadata.service.jdbi3.UserRepository userRepository =
          (org.umetadata.service.jdbi3.UserRepository)
              org.umetadata.service.Entity.getEntityRepository(
                  org.umetadata.service.Entity.USER);

      // First get the bot entity
      org.umetadata.schema.entity.Bot bot =
          botRepository.getByName(
              null,
              org.umetadata.service.Entity.INGESTION_BOT_NAME,
              new org.umetadata.service.util.EntityUtil.Fields(java.util.Set.of()));
      ...
      org.umetadata.schema.entity.teams.User botUser =
          userRepository.getByName(
              null,
              bot.getBotUser().getFullyQualifiedName(),
              new org.umetadata.service.util.EntityUtil.Fields(
      ...
      org.umetadata.schema.entity.teams.AuthenticationMechanism authMechanism =
          botUser.getAuthenticationMechanism();
      if (authMechanism.getAuthType()
          != org.umetadata.schema.entity.teams.AuthenticationMechanism.AuthType.JWT) {
        LOG.error(BOT_AUTH_NOT_JWT_ERROR, authMechanism.getAuthType());
        return null;
      }

      org.umetadata.schema.auth.JWTAuthMechanism jwtAuth =
          (org.umetadata.schema.auth.JWTAuthMechanism) authMechanism.getConfig();
  ...
  private Map<String, String> buildLabels(IngestionPipeline pipeline, String runId) {
    ...
    labels.put(LABEL_APP, LABEL_VALUE_UMETADATA);
    labels.put(LABEL_COMPONENT, LABEL_VALUE_INGESTION);
    labels.put(LABEL_MANAGED_BY, LABEL_VALUE_UMETADATA);
