...
from metadata.great_expectations.utils.umeta_config_handler import render_template
...
def test_get_table_entity(input, expected, mocked_umeta, mocked_ge_data_context):
    """Test get table entity"""

    umeta_validation = UMetadataValidationAction(
        data_context=mocked_ge_data_context,
        config_file_path="my/config/path",
        database_service_name=input,
    )

    res = umeta_validation._get_table_entity("database", "schema", "table")
...
def test_get_table_entity_database_service_name(
    input, expected, mocked_umeta, mocked_ge_data_context
):
    """Test get table entity"""
...
def test_table_config_map_initialization(
    mocked_umeta, mocked_ge_data_context, table_config_map_fixture
):
    """Test that expectation_suite_table_config_map parameter works"""
    action = UMetadataValidationAction(
        data_context=mocked_ge_data_context,
        config_file_path="my/config/path",
...
def test_backward_compatibility_without_config_map(
    mocked_umeta, mocked_ge_data_context
):
