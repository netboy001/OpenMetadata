...
def get_rule_library_test_suite_config(workflow_config, sink_config):
    def inner(entity_fqn: str, test_case_definitions: List[TestCaseDefinition]):
        return {
            "source": {
                "type": "postgres",
                "serviceName": "RuleLibraryTestSuite",
                "sourceConfig": {
...
@dataclass
class RuleLibraryTestParameter:
    entity_fqn: str
...
@pytest.fixture(
    params=[
        RuleLibraryTestParameter(
            ...
            test_case_definition=TestCaseDefinition(
                name="rule_library_customer_ids_greater_than_zero",
        ...
        RuleLibraryTestParameter(
            entity_fqn="{database_service_fqn}.dvdrental.public.customer",
            test_case_definition=TestCaseDefinition(
                name="rule_library_customer_ids_greater_than_max",
                testDefinitionName="columnRuleLibrarySqlExpressionValidator",
                columnName="customer_id",
                parameterValues=[{"name": "minValue", "value": "99999"}],
            ),
...
def rule_library_parameters(request, db_service):
...
def test_rule_library_sql_expression_validator(
    ...
    cleanup_fqns,
    rule_library_test_definition,
):
    """Test the Rule Library SQL Expression validator with various scenarios.
    ...
    test_suite_config = get_rule_library_test_suite_config(
        rule_library_parameters.entity_fqn,
        [rule_library_parameters.test_case_definition],
    )

    run_workflow(TestSuiteWorkflow, test_suite_config)

    test_case_fqn = (
        f"{rule_library_parameters.entity_fqn}."
        f"{rule_library_parameters.test_case_definition.columnName}."
        f"{rule_library_parameters.test_case_definition.name}"
    )
