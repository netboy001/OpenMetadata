...
class BaseColumnValueMinToBeBetweenValidator(BaseTestValidator):
    ...
    MIN_BOUND = "minValueForMinInCol"
    MAX_BOUND = "maxValueForMinInCol"
    ...
    def _get_metrics_to_compute(self, test_params: Optional[dict] = None) -> dict:
        ...
        Returns the final metrics to include in dimensional query results.
        For simple validators like Min, this is just the primary metric.

        Args:
            test_params: Optional test parameters (unused for min validator)
    ...
    def _evaluate_test_condition(
        ...
        """Evaluate the min-to-be-between test condition

        For min test, the condition passes if the min value is within the specified bounds.
        ...
        Args:
            metric_values: Dictionary with keys from Metrics enum names
                          e.g., {"MIN": 42.5}
            test_params: Dictionary with 'minValueForMinInCol' and 'maxValueForMinInCol'

        Returns:
            dict with keys:
                - matched: bool - whether test passed (min within bounds)
        ...
        matched = min_bound <= min_value <= max_bound
    ...
    def _format_result_message(
        ...
        """Format the result message for min-to-be-between test
        ...
        if dimension_info:
            return (
                ...
                f"Found min={min_value} vs. the expected min={min_bound}, max={max_bound}"
            )
        else:
            return f"Found min={min_value} vs. the expected min={min_bound}, max={max_bound}."

    def _get_test_result_values(self, metric_values: dict) -> List[TestResultValue]:
        """Get test result values for min-to-be-between test
