...
"""
UMetadata is the high level Python API that serves as a wrapper
...
working with UMetadata entities.
"""
import traceback
...
from metadata.ingestion.umeta.client import REST, APIError, ClientConfig
from metadata.ingestion.umeta.mixins.csv_mixin import CSVMixin
from metadata.ingestion.umeta.mixins.custom_property_mixin import (
    UMetaCustomPropertyMixin,
)
from metadata.ingestion.umeta.mixins.dashboard_mixin import UMetaDashboardMixin
from metadata.ingestion.umeta.mixins.data_contract_mixin import UMetaDataContractMixin
from metadata.ingestion.umeta.mixins.data_insight_mixin import DataInsightMixin
from metadata.ingestion.umeta.mixins.domain_mixin import UMetaDomainMixin
from metadata.ingestion.umeta.mixins.es_mixin import ESMixin
from metadata.ingestion.umeta.mixins.file_mixin import UMetaFileMixin
from metadata.ingestion.umeta.mixins.ingestion_pipeline_mixin import (
    UMetaIngestionPipelineMixin,
)
from metadata.ingestion.umeta.mixins.logs_mixin import UMetaLogsMixin
from metadata.ingestion.umeta.mixins.mlmodel_mixin import UMetaMlModelMixin
from metadata.ingestion.umeta.mixins.patch_mixin import UMetaPatchMixin
from metadata.ingestion.umeta.mixins.pipeline_mixin import UMetaPipelineMixin
from metadata.ingestion.umeta.mixins.profile_mixin import UMetaProfileMixin
from metadata.ingestion.umeta.mixins.query_mixin import UMetaQueryMixin
from metadata.ingestion.umeta.mixins.role_policy_mixin import UMetaRolePolicyMixin
from metadata.ingestion.umeta.mixins.search_index_mixin import UMetaSearchIndexMixin
from metadata.ingestion.umeta.mixins.server_mixin import UMetaServerMixin
from metadata.ingestion.umeta.mixins.service_mixin import UMetaServiceMixin
from metadata.ingestion.umeta.mixins.suggestions_mixin import UMetaSuggestionsMixin
from metadata.ingestion.umeta.mixins.table_mixin import UMetaTableMixin
from metadata.ingestion.umeta.mixins.tag_glossary_mixin import UMetaTagGlossaryMixin
from metadata.ingestion.umeta.mixins.tests_mixin import UMetaTestsMixin
from metadata.ingestion.umeta.mixins.topic_mixin import UMetaTopicMixin
from metadata.ingestion.umeta.mixins.user_mixin import UMetaUserMixin
from metadata.ingestion.umeta.mixins.version_mixin import UMetaVersionMixin
from metadata.ingestion.umeta.models import EntityList
from metadata.ingestion.umeta.routes import ROUTES
from metadata.ingestion.umeta.sse_client import SSEClient
from metadata.ingestion.umeta.utils import (
    decode_jwt_token,
    get_entity_type,
    model_str,
    quote,
...
class CaseInsensitiveEnvSettingsSource(EnvSettingsSource):
    ...
    This allows both exact camelCase (e.g., UMETADATA__connection__hostPort)
    and all uppercase (e.g., UMETADATA__CONNECTION_HOSTPORT) to work.
    """
...
class UMetadataSettings(BaseSettings):
    """UMetadataConnection settings wrapper"""

    model_config = SettingsConfigDict(
        env_prefix="UMETADATA__", env_nested_delimiter="__"
    )

    connection: UMetadataConnection
...
class UMetadata(
    ...
    UMetaUserMixin,
    UMetaQueryMixin,
    UMetaRolePolicyMixin,
    UMetaSearchIndexMixin,
    UMetaCustomPropertyMixin,
    UMetaSuggestionsMixin,
    UMetaDomainMixin,
    UMetaProfileMixin,
    UMetaTagGlossaryMixin,
    Generic[T, C],
):
    """
    Generic interface to the UMetadata API
    ...
    _auth_provider: UMetadataAuthenticationProvider
    ...
    def __init__(
        ...
        self._auth_provider = UMetadataAuthenticationProvider.create(self.config)
    ...
    def log_user_name_from_jwt_token(self) -> None:
        ...
        """
        # Log user name from JWT token if authProvider is umetadata
        if (
            self.config.authProvider
            and self.config.authProvider.value == "umetadata"
        ):
            try:
    ...
    @classmethod
    def from_env(cls) -> "UMetadata":
        settings = UMetadataSettings()
