...
def get_credentials(
    ...
    Returns:
        Credentials
    """
    oauth = oauth or os.environ.get("UMETA_API_OAUTH_TOKEN")

    key_id = key_id or os.environ.get("UMETA_API_KEY_ID")
    if key_id is None and oauth is None:
        raise ValueError(
            "Key ID must be given to access Alpaca trade API",
            " (env: UMETA_API_KEY_ID)",
        )

    secret_key = secret_key or os.environ.get("UMETA_API_SECRET_KEY")
    if secret_key is None and oauth is None:
        raise ValueError(
            "Secret key must be given to access Alpaca trade API"
            " (env: UMETA_API_SECRET_KEY"
        )
