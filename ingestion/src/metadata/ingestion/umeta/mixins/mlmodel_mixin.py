...
from metadata.ingestion.umeta.mixins.lineage_mixin import UMetaLineageMixin
from metadata.ingestion.umeta.utils import format_name
from metadata.utils.logger import umeta_logger

logger = umeta_logger()


class UMetaMlModelMixin(UMetaLineageMixin):
    """
    UMetadata API methods related to MlModel.
    ...
    def get_mlmodel_sklearn(
        ...
        :return: UMetadata CreateMlModelRequest Entity
        """
        try:
            ...
            logger.error(
                ...
                "pip install umetadata-ingestion[sklearn], %s",
                err,
            )
            raise err
