...
from metadata.ingestion.umeta.utils import model_str, quote
from metadata.utils.logger import umeta_logger

logger = umeta_logger()


class UMetaTestsMixin:
    """
    UMetadata API methods related to Tests.
    ...
    def get_executable_test_suite(self, table_fqn: str) -> Optional[TestSuite]:
        ...
        if not table_entity:
            raise RuntimeError(
                f"Unable to find table {table_fqn} in UMetadata. "
    ...
    def get_or_create_executable_test_suite(
        ...
        if not table_entity:
            raise RuntimeError(
                f"Unable to find table {entity_fqn} in UMetadata. "
                "This could be because the table has not been ingested yet or your JWT Token is expired or missing."
            )
