...
This mixin provides methods to send ingestion logs to UMetadata server's
S3 storage backend. It's designed to be used by UMetadata class and
...
class UMetaLogsMixin:
    """
    UMetadata API methods for log streaming to S3.

    This mixin provides centralized methods for shipping logs to the
    UMetadata server's S3 storage backend. It reuses the existing
    UMetadata client instance to avoid creating new sessions.
    """
    ...
    def send_logs_to_s3(
        ...
        """
        Send logs to S3 storage via UMetadata server endpoint.
    ...
    def send_logs_batch(
        ...
        """
        Send logs batch to S3 storage via UMetadata server endpoint with retry logic.
