...
class UMetadataAuthenticationProvider(AuthenticationProvider):
    """
    UMetadata authentication implementation
    ...
    def __init__(self, config: UMetadataConnection):
        self.config = config
        self.security_config: UMetadataJWTClientConfig = self.config.securityConfig
        self.jwt_token = None
        self.expiry = datetime.now() - relativedelta(years=1)

    @classmethod
    def create(cls, config: UMetadataConnection):
        return cls(config)
