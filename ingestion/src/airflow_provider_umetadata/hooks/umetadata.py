...
an UMetadata server and use it for your
operators.
"""
...
class UMetadataHook(BaseHook):
    """
    Airflow hook to store and use an `UMetadataConnection`
    """

    conn_name_attr: str = "umetadata_conn_id"
    default_conn_name = "umetadata_default"
    conn_type = "umetadata"
    hook_name = "UMetadata"

    def __init__(self, umetadata_conn_id: str = default_conn_name) -> None:
        super().__init__()
        self.umetadata_conn_id = umetadata_conn_id
        # Add defaults
    ...
    def get_conn(self) -> UMetadataConnection:
        conn: Connection = self.get_connection(self.umetadata_conn_id)
    ...
    def test_connection(self):
        """Test that we can instantiate the umeta client with the given connection"""
        try:
            UMetadata(self.get_conn())
