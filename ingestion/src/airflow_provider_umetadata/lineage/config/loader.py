...
from airflow_provider_umetadata.lineage.config.commons import LINEAGE
...
def parse_airflow_config(
    ...
    return AirflowLineageConfig(
        ...
        metadata_config=UMetadataConnection(
            hostPort=conf.get(
                LINEAGE,
                "umetadata_api_endpoint",
                fallback="http://localhost:8585/api",
            ),
            authProvider=AuthProvider.umetadata.value,
...
def get_lineage_config() -> AirflowLineageConfig:
    ...
    a JSON file path configures as env in UMETADATA_LINEAGE_CONFIG
    ...
    umetadata_config_file = os.getenv("UMETADATA_LINEAGE_CONFIG")

    # If config file, parse the JSON config, that should conform to AirflowLineageConfig
    if umetadata_config_file:
        with open(umetadata_config_file, encoding="utf-8") as config_file:
