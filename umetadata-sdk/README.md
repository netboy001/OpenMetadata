...
A modern Java SDK for UMetadata that provides a fluent API for all operations.

## Installation

Add the UMetadata SDK to your project:

```xml
<dependency>
  <groupId>org.u-metadata</groupId>
  <artifactId>umetadata-sdk</artifactId>
...
import org.umetadata.sdk.entities.*;
import org.umetadata.sdk.api.*;

// Configure the client
UMetadataConfig config = UMetadataConfig.builder()
    .serverUrl("http://localhost:8585")
    .apiKey("your-api-key")
    .build();

// Initialize the client
UMetadataClient.initialize(config);

// Set default client for static APIs
Table.setDefaultClient(UMetadataClient.getInstance());
User.setDefaultClient(UMetadataClient.getInstance());
Search.setDefaultClient(UMetadataClient.getInstance());
Lineage.setDefaultClient(UMetadataClient.getInstance());
Bulk.setDefaultClient(UMetadataClient.getInstance());
```
...
The SDK provides the same fluent API for all 40+ UMetadata entity types:
...
The UMetadataClient is thread-safe and can be shared across multiple threads. The static API methods use a shared default client instance.
