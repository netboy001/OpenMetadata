...
import org.umetadata.sdk.exceptions.AuthenticationException;
import org.umetadata.sdk.exceptions.ConflictException;
import org.umetadata.sdk.exceptions.InvalidRequestException;
import org.umetadata.sdk.exceptions.UMetadataException;
import org.umetadata.sdk.exceptions.RateLimitException;
import org.umetadata.sdk.models.ErrorResponse;

public class UMetadataHttpClient implements HttpClient {
  ...
  public UMetadataHttpClient(UMetadataConfig config) {
    this.config = config;
    this.objectMapper =
  ...
  public <T> T execute(
    ...
    try (Response response = okHttpClient.newCall(request).execute()) {
      ...
      throw new UMetadataException("Network error: " + e.getMessage(), e);
    }
  }
            ...
            new Callback() {
              ...
              public void onFailure(Call call, IOException e) {
                future.completeExceptionally(
                    new UMetadataException("Network error: " + e.getMessage(), e));
              }
  ...
  private <T> T handleResponse(Response response, Class<T> responseClass)
    ...
    if (response.isSuccessful()) {
      ...
      try {
        ...
        throw new UMetadataException("Failed to parse response: " + e.getMessage(), e);
      }
    } else {
  ...
  private String handleStringResponse(Response response) throws UMetadataException {
    if (response.isSuccessful()) {
      ResponseBody responseBody = response.body();
      if (responseBody == null) {
        return null;
      ...
      try {
        ...
        throw new UMetadataException("Failed to read response: " + e.getMessage(), e);
      }
    } else {
      handleErrorResponse(response);
      return null; // This line will never be reached due to exception
  ...
  private void handleErrorResponse(Response response) throws UMetadataException {
