package org.umetadata.sdk.examples;
...
public class BuilderEndToEnd {
  ...
  public static void main(String[] args) {
    UMetadataConfig cfg =
        UMetadataConfig.builder().serverUrl(SERVER_URL).accessToken(TOKEN).build();
    UMetadataClient client = new UMetadataClient(cfg);

    /* org.umetadata.sdk.fluent.Glossaries.setDefaultClient(client);

    org.umetadata.schema.api.services.CreateDatabaseService svcReq =
        new DatabaseServiceBuilder(client)
            .name("mysql_prod_demo2")
            .connection(
                org.umetadata.sdk.fluent.DatabaseServices.mysqlConnection()
    ...
    org.umetadata.schema.api.data.CreateTable tableReq =
        new TableBuilder(client)
            .name("customers")
            .description("Customer master table")
            .schemaFQN(schemaFqn)
            .addColumn("id", "BIGINT")
            .column(org.umetadata.sdk.fluent.builders.ColumnBuilder.varchar("email", 255))
    ...
    Table updated =
        new org.umetadata.sdk.fluent.wrappers.FluentTable(
    ...
    try {
      String csv = org.umetadata.sdk.fluent.Glossaries.exportCsv(glossaryName).toCsv();
      System.out.printf("Exported glossary '%s' CSV length=%d%n", glossaryName, csv.length());

      String dryRun =
          org.umetadata.sdk.fluent.Glossaries.importCsv(glossaryName)
              .withData(csv)
              .dryRun()
              .execute();
      ...
      String applied =
          org.umetadata.sdk.fluent.Glossaries.importCsv(glossaryName).withData(csv).execute();
      ...
      java.util.concurrent.CompletableFuture<String> job =
          org.umetadata.sdk.fluent.Glossaries.exportCsv(glossaryName)
              .async()
