...
public class CustomProperties {
  ...
  public static class CustomPropertyUpdater<T> {
    ...
    public T execute() {
      UMetadataClient client = getClient(entityClass);
    ...
    @SuppressWarnings("unchecked")
    private T fetchEntity(UMetadataClient client) {
    ...
    @SuppressWarnings("unchecked")
    private T updateEntity(UMetadataClient client, T entity) {
      try {
        ...
        } else if (entity instanceof org.umetadata.schema.entity.data.Glossary) {
          return (T)
              client
                  .glossaries()
                  .update(
                      entityId.toString(), (org.umetadata.schema.entity.data.Glossary) entity);
        }
        // Add more entity types as needed
    ...
    private UMetadataClient getClient(Class<?> entityClass) {
      try {
        // Use reflection to get the default client from the entity class
        var method = entityClass.getDeclaredMethod("getClient");
        method.setAccessible(true);
        return (UMetadataClient) method.invoke(null);
      } catch (Exception e) {
        throw new RuntimeException("Failed to get UMetadata client", e);
      }
    }
  }
}
