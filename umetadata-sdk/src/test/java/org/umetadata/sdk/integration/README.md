...
The `.java.disabled` files in this directory are integration tests that require a running UMetadata server, database, and search infrastructure.
...
- Use Mockito to mock the UMetadataClient and service APIs
...
### Integration Tests (In umetadata-service)
Real integration tests that require a running server should be placed in the `umetadata-service` module where they can:
...
To enable these tests for running in umetadata-service:
1. Move the `.java.disabled` files to `umetadata-service/src/test/java/org/umetadata/service/resources/`
2. Extend `EntityResourceTest` instead of `BaseSDKTest`
3. Use the test infrastructure provided by umetadata-service
